...
export const getClaimConditionMeaning = (c: Options.ClaimCondition) => {
  const qLang1 = (q: "lt" | "gt" | "eq") => q == "gt" ? "more than" : q == "eq" ? "exactly" : "less than"
  const qLang2 = (q: "lt" | "gt" | "eq") => q == "gt" ? "at least" : q == "eq" ? "exactly" : "at most"
  ...
  const evaluate: { [key: string]: (c: Options.ClaimCondition) => string } = {
    ...
    accuracy: (c) => `Get ${qLang1(c.quantifier ?? "gt")} ${(parseFloat(c.value).toFixed(2))}% accuracy`,
