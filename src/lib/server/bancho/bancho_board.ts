...
const banchoBoard = (card: Bingo.Card): string[][] | null => {
  ...
  for (let i = 0; i < card.squares.length; i++) {
    ...
    output[square.y_pos][square.x_pos] = linkify(square, i + 1)
  }
  return output;
}

const linkify = (square: Bingo.Card.FullSquare, index: number) =>
  `[https://osu.ppy.sh/b/${square.map_id} [${index < 10 ? `0${index}` : index}]]`