...
export const finalCall = async (game_id: string) => {
  ...
  if ((total_claimed / count.length) != count[0].count) {
    ...
    logger.info(`Game ${game_id} resulted in a final call tiebreaker, won by ${winning_team}`, { ...game, count })
  ...
  if ((total_score / score_map.length) != score_map[0].sum) {
    ...
    logger.info(`Game ${game_id} resulted in a final call score tiebreaker, won by ${winning_team}`, { ...game, count: score_map })
    q.setGameState(game_id, 2, winning_team);
    sendToGame(game_id, {
      type: 'state', data: {
        state: 2,
