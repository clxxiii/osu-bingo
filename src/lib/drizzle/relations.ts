...
export const BingoSquareRelations = relations(BingoSquare, ({ one, many }) => ({
	scores: many(Score),
	game: one(BingoGame, {
		fields: [BingoSquare.game_id],
		references: [BingoGame.id]
	}),
	map: one(Map, {
		fields: [BingoSquare.map_id],
		references: [Map.id]
	})
}));
...
export const GameUserRelations = relations(GameUser, ({ one }) => ({
	game: one(BingoGame, {
		fields: [GameUser.game_id],
		references: [BingoGame.id]
	}),
	user: one(User, {
		fields: [GameUser.user_id],
		references: [User.id]
	})
}));
