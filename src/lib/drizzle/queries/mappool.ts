...
export const getRandomMaps = async (
  ...
  if (max_sr)
    restrictions.push(lte(MapStats.star_rating, max_sr))
  if (min_sr)
    restrictions.push(gte(MapStats.star_rating, min_sr))
  if (max_length)
    restrictions.push(lte(MapStats.length, max_length))
  if (min_length)
    restrictions.push(gte(MapStats.length, min_length))

  logger.silly("Started db request", { "function": "getRandomMaps", "obj": "select", "dir": "start" })
  const select = await db.select({ id: Map.id })
    .from(Map)
