...
export const GameUser = sqliteTable(
    'GameUser', {
      game_id : text('game_id').notNull().references(
          () => BingoGame.id, {onDelete : 'cascade', onUpdate : 'cascade'}),
      user_id : integer('user_id').notNull().references(
...
export const MapStats =
    sqliteTable('MapStats', {
      map_id : integer('map_id').notNull().references(
          () => Map.id, {onDelete : 'cascade', onUpdate : 'cascade'}),
      ...
      hp : real('hp').notNull()
                ...
                (table) => ({
                  composite_key : primaryKey(
                      {columns : [ table.map_id, table.mod_string ]}),
...
export const Score =
    sqliteTable('Score', {
      ...
      square_id : text('square_id').notNull().references(() => BingoSquare.id, {
        onDelete : 'cascade',
        onUpdate : 'cascade'
      }),
