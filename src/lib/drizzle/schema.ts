 ...
 *
 * Contains references to multiple users, multiple maps (bingo squares), and multiple scores.
 */
export const BingoGame = sqliteTable('BingoGame', {
	...
	// Settings
	state: integer('state').default(0).notNull(), // 0: Before starting, 1: In game, 2: Finished
	allow_team_switching: integer('allow_team_switching', { mode: 'boolean' }).default(true) // Only takes effect when game state is 0.
});
...
export const User = sqliteTable('User', {
	...
	last_refreshed: integer('last_refreshed', { mode: 'timestamp' }).$defaultFn(() => new Date())
});
...
export const TimeEvent = sqliteTable('TimeEvent', {
	...
	action: text('action').notNull(), // Some string that represents what to do when the time hits
 ...
 *
 * Linked to a Map, with some mod_string. For displaying NM stats, DT stats, etc.
 * mod_string should match the string of the MapInPool object ("" means nomod)
 */
...
export const Score = sqliteTable('Score', {
	...
	important: integer('important', { mode: 'boolean' }).default(false), // Whether this score appears as a notification

	square_id: text('square_id')
		.notNull()
		.references(() => BingoSquare.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
	user_id: integer('user_id')
		.notNull()
		.references(() => GameUser.user_id, { onDelete: 'cascade', onUpdate: 'cascade' })
});
