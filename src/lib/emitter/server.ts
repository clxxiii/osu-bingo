...
export const addListener =
    (fn: Fn, game_id?: string, user_id?: number, channel?: string) => {
      ...
      logger.info(
          `[+] New Listener ${id}; Total Listeners: ${listeners.size}`,
          {listener, type : 'listener_added', listener_count : listeners.size})
      return listener;
    }
...
export const removeListener =
    (id: string) => {
      ...
      if (listener.channel && listener.game_id) {
        const game_channel =
            game_channels.get(`${listener.game_id}_${listener.channel}`);
        if (game_channel) {
          const i = game_channel.indexOf(id);
          if (i != -1)
            game_channel.splice(i);

          if (game_channel.length == 0)
            game_channels.delete(`${listener.game_id}_${listener.channel}`)
            else game_channels.set(`${listener.game_id}_${listener.channel}`,
                                   game_channel);
        }
      }

      logger.info(
          `[-] Listener ${id} removed; Total Listeners: ${listeners.size}`, {
            listener,
...
export const sendToListener =
    (id: string, event: EmitterEvent) => {
      const listener = listeners.get(id);
      if (listener)
        listener.fn(event);
    }
...
export const sendToGame =
    (game_id: string, event: EmitterEvent) => {
      ...
      logger.info(`Sent ${event.type} event to game ${game_id}`,
                  {event, type : 'sent_game_event'})
    }

export const sendToUser =
    (user_id: number, event: EmitterEvent) => {
      ...
      logger.info(`Sent ${event.type} event to user ${user_id}`,
                  {event, type : 'sent_user_event'})
    }

export const sendToChannel =
    (game_id: string, channel: string, event: EmitterEvent) => {
      ...
      logger.info(
          `Sent ${event.type} event to game channel ${game_id}_${channel}`,
          {event, type : 'sent_channel_event'})
    }
...
export const switchChannel = (id: string, channel: string) => {
  ...
  logger.info(`[~] Switched listener ${id} to channel ${channel}`,
              {listener, type : 'listener_channel_switch'})
  return listener;
}
