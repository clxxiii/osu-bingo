...
export const setUser = async (user: typeof User.$inferInsert) => {
	...
	const userUpdate = (
		await db.update(User).set(newUser).where(eq(User.id, user.id)).returning()
	)[0];
...
export const getUserFromSessionToken = async (token: string) => {
	...
	logger.silly('Finished db request', {
		function: 'getUserFromSessionToken',
		obj: 'tokens',
		dir: 'end'
	});
...
export const updateUser = async (token: Bingo.OauthToken) => {
	const response = await refreshOAuthToken(token, pub_env.PUBLIC_OSU_CLIENT_ID, env.OSU_CLIENT_SECRET);
	...
	if (user.statistics) {
		...
		logger.info(
			`Successfully updated user info for ${user.id} (${user.username}: #${user?.statistics?.global_rank?.toLocaleString()})`,
			{ type: 'token_update' }
		);
		return getToken(user.id, 'osu');
	} else {
		logger.info(`Failed trying to update user info for token ${token.id} (${token.user_id})`, {
