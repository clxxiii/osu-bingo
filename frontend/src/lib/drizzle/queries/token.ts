...
export const setToken = async (token: typeof OauthToken.$inferInsert) => {
	...
	const dbToken = await db
		.select()
		.from(OauthToken)
		.where(and(eq(OauthToken.user_id, token.user_id), eq(OauthToken.service, token.service)));
...
export const getLingeringTokens = async () => {
	...
	const lingeringTokens = await db
		.select()
		.from(OauthToken)
		.where(and(lte(OauthToken.expires_at, cutoff), eq(OauthToken.service, 'osu')));
...
export const getToken = async (user_id: number, service?: string) => {
	...
	const token = (
		await db
			.select()
			.from(OauthToken)
			.where(and(eq(OauthToken.user_id, user_id), eq(OauthToken.service, service ?? 'osu')))
	)[0];
