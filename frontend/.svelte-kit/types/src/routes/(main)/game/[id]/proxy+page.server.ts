...
export const actions = {
	join_game: async ({ request, params, locals }: import('./$types').RequestEvent) => {
	...
	switch_team: async ({ request, params, locals }: import('./$types').RequestEvent) => {
		const form = await request.formData();
		const linkId = params.id;
		const team = form.get('team');
	...
	leave_game: async ({ params, locals }: import('./$types').RequestEvent) => {
		const linkId = params.id;
		const user = locals.user;
		const game = await q.gameExists(linkId);
	...
	chat: async ({ params, request, locals }: import('./$types').RequestEvent) => {
		const user = locals.user;
		const linkId = params.id;
		const game = await q.gameExists(linkId);
	...
	send_board: async ({ params, locals }: import('./$types').RequestEvent) => {
		const user = locals.user;
		const linkId = params.id;
	...
	start_game: async ({ params, locals }: import('./$types').RequestEvent) => {
		const user = locals.user;
		const linkId = params.id;
		const game_check = await q.gameExists(linkId);
	...
	change_settings: async ({ params, locals, request }: import('./$types').RequestEvent) => {
		const form = await request.formData();
		const linkId = params.id;
		const actor = locals.user;
		const game_check = await q.gameExists(linkId);
	...
	delete_game: async ({ params, locals }: import('./$types').RequestEvent) => {
		const user = locals.user;
		const linkId = params.id;
		const game_check = await q.gameExists(linkId);
